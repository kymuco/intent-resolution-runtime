...
def _persist(repository: InMemoryAdmittedHistoryRepository, record: object) -> None:
    canonical_bytes = record.canonical_bytes()  # type: ignore[attr-defined]
...
class RecordingProvider:
    ...
    def propose(self, request: object) -> CandidateResolution:
        self.calls += 1
        return CandidateResolution(
            intent_request_identity=request.intent_request_identity,  # type: ignore[attr-defined]
            context_envelope_identity=request.context_envelope_identity,  # type: ignore[attr-defined]
            attribution=CandidateAttribution(
                provider_ref=request.provider_ref,  # type: ignore[attr-defined]
                invocation_ref=request.invocation_ref,  # type: ignore[attr-defined]
            ),
            proposed_semantics=(
...
def _admit_work_plan(
    ...
    unresolved = orchestrate_work_disposition(
        resolved,
...
def _admit_requirement(work_plan: WorkPlan) -> AdmittedCapabilityRequirement:
    ...
    candidate = CandidateCapabilityRequirement(
        ...
        rationale="The exact publication step requires the bounded publication capability.",
    )
...
def _admit_catalog(
    ...
    descriptor_boundary = CapabilityExecutionBoundary(
    ...
    descriptor = CapabilityDescriptor(
        ...
        execution_boundaries=(descriptor_boundary,),
    ...
    snapshot = CapabilityCatalogSnapshot(
        ...
        scope="M3.6 exact bounded publication fixture catalog.",
...
class RecordingGovernance:
    ...
    def review(self, request: object) -> GovernanceDecision:
        self.calls += 1
        proposal = request.proposal  # type: ignore[attr-defined]
        return GovernanceDecision(
            attribution=GovernanceDecisionAttribution(
                governance_ref=request.governance_ref,  # type: ignore[attr-defined]
                decision_event_ref=request.decision_event_ref,  # type: ignore[attr-defined]
                authority_context_ref=request.authority_context_ref,  # type: ignore[attr-defined]
                authority_context_identity=request.authority_context_identity,  # type: ignore[attr-defined]
            ),
            proposal=proposal,
            components=(
                GovernanceDecisionComponent(
...
class RecordingExecutor:
    ...
    def invoke(self, request: object) -> CapabilityOutcome:
        self.calls += 1
        attempt = request.attempt  # type: ignore[attr-defined]
        lifecycle = OutcomeEvidence(
            ...
            description="The exact executor protocol completed normally.",
        )
        receipt = OutcomeEvidence(
            ...
            temporal_basis_refs=(OUTCOME_TEMPORAL_IDENTITY,),
            scope="artifact.publish attempt",
            description="Attributable evidence confirms the exact publication.",
        )
        ...
        return CapabilityOutcome(
            ...
            effects=(
                OutcomeEffectAssessment(
                    ...
                    evidence_refs=(receipt.evidence_ref,),
                    rationale="The requested publication effect is confirmed.",
                ),
            ),
...
def _delegation(resolved: ResolvedIntent, parent: WorkPlan) -> DelegatedWork:
    ...
    return DelegatedWork(
        ...
        objective="Inspect the bounded publication context and return an explicit need if evidence is insufficient.",
        scopes=(scope,),
        context_surface=(),
        allowed_capabilities=(),
        constraints=(),
        expected_deliverables=(deliverable,),
        completion_contract="Return the exact audit note or an explicit bounded WorkerNeed.",
...
def test_m3_6_capability_lane_composes_and_replays_without_external_reexecution() -> None:
    ...
    assert resolution_frontier.kind is InitialResolutionFrontierKind.RESOLUTION_OUTPUT_AVAILABLE
    ...
    derived_evaluation = derive_mechanical_capability_match_evaluation(
        admitted_requirement,
        admitted_catalog,
        evaluation_event_ref=_ref("irr.event", "m3-6-capability-evaluation"),
    )
    evaluation = derived_evaluation.evaluation
...
def test_m3_6_worker_lane_replays_without_worker_redispatch_or_parent_completion() -> None:
    repository = InMemoryAdmittedHistoryRepository()
    request, context = _request_and_context()
